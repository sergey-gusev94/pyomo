...
logger = logging.getLogger('pyomo.gdp.hull_exact_extra_var_inequal')
...
@TransformationFactory.register(
    ...
    with extra variable reformulation for quadratic constraints using inequality 
    for the linear link: v'Qv <= t*y where t + c'v + (d-ub)*y <= 0 for upper bounds,
    and equality t = -c'v + (bound - d)*y for equality constraints.'''
)
class Hull_Reformulation_Extra_Var_Inequal(GDP_to_MIP_Transformation):
    ...
    For quadratic constraints x'Qx + c'x + d <= ub, instead of the standard
    hull reformulation v'Qv + c'vy + dy² <= ub*y², this transformation uses:
        v'Qv <= t*y
        t + c'v + (d - ub)*y <= 0  (inequality for upper bound)
    where t is an extra auxiliary variable.

    For quadratic constraints x'Qx + c'x + d >= lb:
        -v'Qv <= t*y
        t - c'v + (lb - d)*y <= 0  (inequality for lower bound)

    For equality constraints x'Qx + c'x + d == bound:
        v'Qv == t*y
        t = -c'v + (bound - d)*y  (equality is preserved)

    The inequality formulation for the linear link gives more flexibility
    to the solver compared to the equality formulation.
    ...
    CONFIG = cfg.ConfigDict('gdp.hull_exact_extra_var_inequal')
    CONFIG.declare(
        'targets',
        cfg.ConfigValue(
            default=None,
    ...
    transformation_name = 'hull_exact_extra_var_inequal'
    ...
    def _transform_constraint(
        ...
        for i in sorted(obj.keys()):
            ...
            if general_NL:
                ...
                # For quadratic constraints, use the extra variable reformulation
                # with INEQUALITY for the linear link (for inequality constraints):
                # 
                # Original constraint: x'Qx + c'x + d <= ub (in disjunct)
                # Standard exact hull: v'Qv + c'vy + dy² <= ub·y²
                # 
                # Rewrite as: v'Qv + (c'v + dy)·y <= ub·y²
                #             v'Qv <= (ub·y - c'v - dy)·y
                #             v'Qv <= t·y  where  t >= -c'v + (ub - d)·y
                #
                # The inequality formulation uses:
                #   t + c'v + (d - ub)·y <= 0  (instead of equality)
                #   v'Qv <= t·y
                #
                # For lower bound: v'Qv + c'vy + dy² >= lb·y²
                #   Negate: -v'Qv - c'vy - dy² <= -lb·y²
                #   -v'Qv <= t·y  where  t >= c'v + (d - lb)·y
                #   t - c'v + (lb - d)·y <= 0  (inequality)
                #
                # For equality: v'Qv + c'vy + dy² == bound·y²
                #   v'Qv == t·y  where  t = -c'v + (bound - d)·y (EQUALITY preserved)
                ...
                if repn.is_quadratic():
                    # Build the quadratic form v'Qv
                    quadratic_form = 0
                    for idx, (var_i, var_j) in enumerate(repn.quadratic_vars):
                        coef = repn.quadratic_coefs[idx]
                        ...
                        if var_i is var_j:
                            ...
                            quadratic_form += coef * v_i**2
                        else:
                            # For cross-terms x_i*x_j, the exact hull is v_i*v_j
                            quadratic_form += coef * v_i * v_j
                    
                    # Build the linear part c'v
                    # (the constant part will be added per-bound)
                    linear_part_cv = 0
                    ...
                    for coef, var in zip(lin_coefs, lin_vars):
                        ...
                        linear_part_cv += coef * v
                    
                    # Store quadratic form for constraint generation
                    quad_expr_lhs = quadratic_form
                    
                    # Store linear_part_cv and const_term for use in bound handling
                    # These will be used to construct t constraints
            ...
            if c.equality:
                if general_NL:
                    ...
                    # Equality constraint with quadratic:
                    # v'Qv == t*y  where  t = -c'v + (bound - d)*y
                    # For EQUALITY constraints, we keep the EQUALITY for t definition
                    bound = c.lower  # for equality, lower == upper
                    
                    # Create auxiliary variable t
                    t = Var(within=Reals)
                    t_name = unique_component_name(
                        relaxationBlock,
                        '_aux_t_%s_eq' % c.getname(fully_qualified=True, relative_to=disjunct)
                    )
                    relaxationBlock.add_component(t_name, t)
                    
                    # t = -c'v + (bound - d)*y
                    # Equivalently: t + c'v + (d - bound)*y = 0
                    t_def_expr = t + linear_part_cv + (const_term - bound) * y
                    t_def_constraint_name = unique_component_name(
                        relaxationBlock,
                        '_t_def_%s_eq' % c.getname(fully_qualified=True, relative_to=disjunct)
                    )
                    # EQUALITY constraint for t definition (equality case)
                    t_def_constraint = Constraint(expr=t_def_expr == 0)
                    relaxationBlock.add_component(t_def_constraint_name, t_def_constraint)
                    
                    # Map the t definition constraint
                    constraint_map.transformed_constraints[c].append(t_def_constraint)
                    constraint_map.src_constraint[t_def_constraint] = c
                    
                    # Main constraint: v'Qv == t*y
                    newConsExpr = quad_expr_lhs == t * y
            ...
            if c.lower is not None:
                ...
                if general_NL:
                    ...
                    # Lower bound: v'Qv + c'vy + dy² >= lb*y²
                    # 
                    # To always use v'Qv <= t*y form, we negate everything:
                    # -v'Qv - c'vy - dy² <= -lb*y²
                    # -v'Qv <= (-lb + d)*y² + c'vy
                    # -v'Qv <= (c'v + (d - lb)*y)*y
                    # -v'Qv <= t*y  where  t <= c'v + (d - lb)*y
                    ...
                    #   Bound: t <= c'v + (d - lb)*y
                    #
                    # INEQUALITY formulation: t - c'v + (lb - d)*y <= 0
                    # This ensures t <= c'v + (d - lb)*y
                    bound = c.lower
                    
                    # Create auxiliary variable t
                    t_lb = Var(within=Reals)
                    t_name = unique_component_name(
                        relaxationBlock,
                        '_aux_t_%s_lb' % c.getname(fully_qualified=True, relative_to=disjunct)
                    )
                    relaxationBlock.add_component(t_name, t_lb)
                    
                    # t <= c'v + (d - lb)*y
                    # Equivalently: t - c'v - (d - lb)*y <= 0
                    # Or: t - c'v + (lb - d)*y <= 0
                    t_def_expr = t_lb - linear_part_cv + (bound - const_term) * y
                    t_def_constraint_name = unique_component_name(
                        relaxationBlock,
                        '_t_def_%s_lb' % c.getname(fully_qualified=True, relative_to=disjunct)
                    )
                    # INEQUALITY constraint for t definition (lower bound case)
                    t_def_constraint = Constraint(expr=t_def_expr <= 0)
                    relaxationBlock.add_component(t_def_constraint_name, t_def_constraint)
                    ...
                    # Main constraint: -v'Qv <= t*y
                    newConsExpr = -quad_expr_lhs <= t_lb * y
                else:
                    newConsExpr = expr - (1 - y) * h_0 >= c.lower * y
            ...
            if c.upper is not None:
                ...
                if general_NL:
                    ...
                    # Upper bound: v'Qv + c'vy + dy² <= ub*y²
                    # 
                    # The "sandwich" structure requires:
                    #   Shape: v'Qv <= t*y
                    #   Bound: t <= -c'v + (ub - d)*y
                    #
                    # INEQUALITY formulation: t + c'v + (d - ub)*y <= 0
                    # This ensures t <= -c'v + (ub - d)*y
                    bound = c.upper
                    
                    # Create auxiliary variable t
                    t_ub = Var(within=Reals)
                    t_name = unique_component_name(
                        relaxationBlock,
                        '_aux_t_%s_ub' % c.getname(fully_qualified=True, relative_to=disjunct)
                    )
                    relaxationBlock.add_component(t_name, t_ub)
                    
                    # t <= -c'v + (ub - d)*y
                    # Equivalently: t + c'v - (ub - d)*y <= 0
                    # Or: t + c'v + (d - ub)*y <= 0
                    t_def_expr = t_ub + linear_part_cv + (const_term - bound) * y
                    t_def_constraint_name = unique_component_name(
                        relaxationBlock,
                        '_t_def_%s_ub' % c.getname(fully_qualified=True, relative_to=disjunct)
                    )
                    # INEQUALITY constraint for t definition (upper bound case)
                    t_def_constraint = Constraint(expr=t_def_expr <= 0)
                    relaxationBlock.add_component(t_def_constraint_name, t_def_constraint)
                    ...
                    # Main constraint: v'Qv <= t*y
                    newConsExpr = quad_expr_lhs <= t_ub * y
                else:
