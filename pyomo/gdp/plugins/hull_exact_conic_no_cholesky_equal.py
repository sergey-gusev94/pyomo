...
logger = logging.getLogger('pyomo.gdp.hull_exact_conic_no_cholesky_equal')
...
@TransformationFactory.register(
    'gdp.hull_exact_conic_no_cholesky_equal', 
    doc='''Relax disjunctive model by forming the hull reformulation 
    and using exact hull for quadratic constraints (no extra z variables, no Cholesky factorization, equality for t).'''
)
class Hull_Reformulation(GDP_to_MIP_Transformation):
    ...
    CONFIG = cfg.ConfigDict('gdp.hull_exact_conic_no_cholesky_equal')
    CONFIG.declare(
        'targets',
        cfg.ConfigValue(
            default=None,
    ...
    transformation_name = 'hull_exact_conic_no_cholesky_equal'
        ...
        for i in sorted(obj.keys()):
            ...
            if general_NL:
                ...
                if repn.is_quadratic():
                    ...
                    # Determine Q_for_conic if using conic for either bound
                    # (for lower bound constraints, we need to negate Q)
                    ...
                    if use_conic_upper or use_conic_lower:
                        ...
                        # Add the conic constraint: v'Q_for_conic*v ≤ t*y
                        # This is a rotated second-order cone (RSOC) constraint.
                        # Since Q_for_conic is symmetric and PSD, we can write it directly as:
                        # v'Q_for_conic*v ≤ t * y
                        #
                        ...
                        # sqrt(4*v'Q_for_conic*v + (t-y)²) <= t+y
                        
                        # Build quadratic form v'Q_for_conic*v directly from repn
                        # This matches the structure used in the non-convex case
                        quadratic_form = 0
                        for i, (var_i, var_j) in enumerate(repn.quadratic_vars):
                            coef = repn.quadratic_coefs[i]
                            ...
                            # Apply negation if needed (for lower bound constraints)
                            actual_coef = -coef if negate_for_conic else coef
                            
                            # Check if this is a squared term (x²)
                            if var_i is var_j:
                                # For x², the quadratic form is coef * v²
                                quadratic_form += actual_coef * v_i**2
                            else:
                                # For cross-terms x_i*x_j, the quadratic form is coef * v_i * v_j
                                quadratic_form += actual_coef * v_i * v_j
                        
                        # Build rotated SOC constraint: v'Q_for_conic*v ≤ t*y
                        rsoc_lhs = quadratic_form
                        rsoc_rhs = t * y
            ...
            if c.lower is not None:
                ...
                if polynomial_degree == 2 and use_conic_lower:
                    ...
                    # Original: c.lower ≤ expr  →  -expr = -c.lower (equality for t)
            ...
            if c.upper is not None:
                ...
                if polynomial_degree == 2 and use_conic_upper:
                    # For conic upper bound: t + c'v + dy == c.upper * y (equality for t)
...
@TransformationFactory.register(
    'gdp.chull_equal',
    doc="[DEPRECATED] please use 'gdp.hull_exact_conic_no_cholesky_equal' to get the Hull transformation with equality for t.",
)
@deprecated(
    "The 'gdp.chull_equal' name is deprecated. "
    "Please use the more apt 'gdp.hull_exact_conic_no_cholesky_equal' instead.",
    logger='pyomo.gdp',
    version="5.7",
)
