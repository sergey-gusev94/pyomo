...
logger = logging.getLogger('pyomo.gdp.hull_exact_conic_sqrt_no_extra_var')
...
class Hull_Reformulation(GDP_to_MIP_Transformation):
    ...
    CONFIG = cfg.ConfigDict('gdp.hull_exact_conic_sqrt_no_extra_var')
    CONFIG.declare(
        'targets',
        cfg.ConfigValue(
            default=None,
    ...
    transformation_name = 'hull_exact_conic_sqrt_no_extra_var'
            ...
            if general_NL:
                ...
                if repn.is_quadratic():
                    ...
                    if use_conic_upper or use_conic_lower:
                        ...
                        # Build SOC constraint in standard form: sqrt(...) <= t+y
                        # ||w||₂ ≤ s  where w = (2*(L^T v), t-y) and s = t+y
                        # This is: sqrt(4*sum_k((L^T v)_k²) + (t-y)²) ≤ t+y
                        
                        sum_of_squares = sum((2 * expr_k)**2 for expr_k in linear_exprs)
                        soc_lhs = sqrt(sum_of_squares + (t - y)**2)
                        soc_rhs = t + y
