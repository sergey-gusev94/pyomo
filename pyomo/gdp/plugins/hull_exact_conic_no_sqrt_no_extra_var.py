...
logger = logging.getLogger('pyomo.gdp.hull_exact_conic_no_sqrt_no_extra_var')
...
@TransformationFactory.register(
    ...
    and using exact hull for quadratic constraints (squared SOC form, no extra z variables).'''
)
class Hull_Reformulation(GDP_to_MIP_Transformation):
    ...
    CONFIG = cfg.ConfigDict('gdp.hull_exact_conic_no_sqrt_no_extra_var')
    CONFIG.declare(
        'targets',
        cfg.ConfigValue(
            default=None,
    ...
    transformation_name = 'hull_exact_conic_no_sqrt_no_extra_var'
            ...
            if general_NL:
                ...
                if repn.is_quadratic():
                    ...
                    if use_conic_upper or use_conic_lower:
                        ...
                        # Reformulated as standard SOC (without auxiliary z variables):
                        # sqrt(4*||L^T v||² + (t-y)²) ≤ t+y
                        #
                        # This is equivalent to ||(2*L^T v, t-y)||₂ ≤ t+y
                        # Which represents the rotated SOC: ||L^T v||² ≤ t*y
                        
                        r = L.shape[1]  # number of positive eigenvalues
                        
                        # Build the linear expressions (L^T @ v)_k directly without z variables
                        ...
                        # Build SOC constraint in squared form (no sqrt):
                        # 4*||L^T v||² + (t-y)² ≤ (t+y)²
                        # Equivalent to: 4*sum_k((L^T v)_k²) + (t-y)² ≤ (t+y)²
                        # This simplifies to: ||L^T v||² ≤ t*y (rotated SOC)
                        
                        sum_of_squares = sum((2 * expr_k)**2 for expr_k in linear_exprs)
                        soc_lhs = sum_of_squares + (t - y)**2
                        soc_rhs = (t + y)**2
