...
logger = logging.getLogger('pyomo.gdp.hull_exact_conic_original')
...
@TransformationFactory.register(
    ...
    and using exact hull for quadratic constraints (no extra z variables).'''
)
class Hull_Reformulation(GDP_to_MIP_Transformation):
    ...
    CONFIG = cfg.ConfigDict('gdp.hull_exact_conic_original')
    CONFIG.declare(
        'targets',
        cfg.ConfigValue(
            default=None,
    ...
    transformation_name = 'hull_exact_conic_original'
            ...
            if general_NL:
                ...
                if repn.is_quadratic():
                    ...
                    if use_conic_upper or use_conic_lower:
                        ...
                        # This is a rotated second-order cone (RSOC) constraint.
                        # Written directly as: sum_k((L^T v)_k²) <= t * y
                        #
                        # NOTE: This bilinear form (t*y) may NOT be automatically recognized
                        # as a rotated SOC by most solvers. Only Mosek has explicit RSOC support.
                        # For better solver compatibility, consider using the standard SOC form:
                        # sqrt(4*||L^T v||² + (t-y)²) <= t+y
                        
                        r = L.shape[1]  # number of positive eigenvalues
                        
                        # Build the linear expressions (L^T @ v)_k directly
                        # (L^T v)_k = sum_j L[j,k] * v[j]
                        linear_exprs = []
                        for k in range(r):
                            expr_k = sum(L[j, k] * v_vector[j] for j in range(n_vars))
                            linear_exprs.append(expr_k)
                        
                        # Build rotated SOC constraint: ||L^T v||² ≤ t*y
                        # This is: sum_k((L^T v)_k²) <= t * y
                        sum_of_squares = sum(expr_k**2 for expr_k in linear_exprs)
                        rsoc_lhs = sum_of_squares
                        rsoc_rhs = t * y
                        
                        # Add the rotated conic constraint to the relaxation block
                        conic_constraint_name = unique_component_name(
                            relaxationBlock,
                            '_conic_constraint_%s' % c.getname(fully_qualified=True, relative_to=disjunct)
                        )
                        conic_constraint = Constraint(expr=rsoc_lhs <= rsoc_rhs)
