...
logger = logging.getLogger('pyomo.gdp.hull_exact_conic_sqrt_extra_var')
...
@TransformationFactory.register(
    ...
    and using exact hull for quadratic constraints (sqrt SOC form with auxiliary z, w, s variables).'''
)
class Hull_Reformulation(GDP_to_MIP_Transformation):
    ...
    CONFIG = cfg.ConfigDict('gdp.hull_exact_conic_sqrt_extra_var')
    CONFIG.declare(
        'targets',
        cfg.ConfigValue(
            default=None,
    ...
    transformation_name = 'hull_exact_conic_sqrt_extra_var'
            ...
            if general_NL:
                ...
                if repn.is_quadratic():
                    ...
                    if use_conic_upper or use_conic_lower:
                        ...
                        # Build SOC constraint: sqrt(sum(z[k]²) + w²) ≤ s
                        # Standard SOC form: ||w||₂ ≤ s where w = (z[0], z[1], ..., z[r-1], w_aux)
                        soc_lhs = sqrt(sum(z[k]**2 for k in range(r)) + w**2)
                        soc_rhs = s
