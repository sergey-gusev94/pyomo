...
logger = logging.getLogger('pyomo.gdp.hull_exact_extra_var')
...
@TransformationFactory.register(
    ...
    with extra variable reformulation for quadratic constraints: v'Qv <= t*y where t = -c'v - d*y.'''
)
class Hull_Reformulation_Extra_Var(GDP_to_MIP_Transformation):
    ...
    For quadratic constraints x'Qx + c'x + d <= 0, instead of the standard
    hull reformulation v'Qv + c'vy + dy² <= 0, this transformation uses:
        v'Qv <= t*y
        t = -c'v - d*y
    ...
    CONFIG = cfg.ConfigDict('gdp.hull_exact_extra_var')
    CONFIG.declare(
        'targets',
        cfg.ConfigValue(
            default=None,
    ...
    transformation_name = 'hull_exact_extra_var'
        ...
        for i in sorted(obj.keys()):
            ...
            if general_NL:
                ...
                if repn.is_quadratic():
                    ...
                    # Build the linear part of t: -c'v
                    # (the constant part (bound - d)*y will be added per-bound)
                    linear_part_cv = 0
                    ...
                    # These will be used to construct t = -c'v + (bound - d)*y
            ...
            if c.equality:
                if general_NL:
                    ...
                    # t = -c'v + (bound - d)*y
                    # Equivalently: t + c'v - (bound - d)*y = 0
                    # Or: t + c'v + (d - bound)*y = 0
            ...
            if c.lower is not None:
                ...
                if general_NL:
                    ...
                    #
                    # So the constraint becomes: -v'Qv <= t*y (i.e., v'Qv >= -t*y)
                    # with t = c'v + (d - lb)*y
                    bound = c.lower
                    ...
                    # Main constraint: -v'Qv <= t*y (equivalently: v'Qv + t*y >= 0)
                    # Written as: -quad_expr_lhs <= t_lb * y
            ...
            if c.upper is not None:
                ...
                if general_NL:
                    ...
                    # Reformulation: v'Qv <= t*y  where  t = -c'v + (ub - d)*y
                    bound = c.upper
