...
from pyomo.core.expr.numvalue import ZeroConstant, NumericConstant
...
logger = logging.getLogger('pyomo.gdp.hull_exact_conic_no_sqrt_extra_var')
...
@TransformationFactory.register(
    ...
    and using exact hull for quadratic constraints (squared SOC form with auxiliary z and s variables).'''
)
class Hull_Reformulation(GDP_to_MIP_Transformation):
    ...
    CONFIG = cfg.ConfigDict('gdp.hull_exact_conic_no_sqrt_extra_var')
    CONFIG.declare(
        'targets',
        cfg.ConfigValue(
            default=None,
    ...
    transformation_name = 'hull_exact_conic_no_sqrt_extra_var'
    ...
    def _transform_constraint(
        ...
        for i in sorted(obj.keys()):
            ...
            # Initialize variables that may be used in inequality branches
            # These are only defined inside the polynomial_degree == 2 block
            ...
            if general_NL:
                ...
                # Get the constant term (d) - keep it symbolic (could be Param)
                ...
                if repn.is_quadratic():
                    ...
                    # Build Q matrix from quadratic coefficients
                    # Important: Build symmetric Q with proper accumulation
                    for coef, (var_i, var_j) in zip(repn.quadratic_coefs, repn.quadratic_vars):
                        # Get matrix indices for these variables
                        idx_i = var_to_idx[id(var_i)]
                        idx_j = var_to_idx[id(var_j)]
                        
                        # Fill Q matrix symmetrically
                        if var_i is var_j:
                            # Diagonal term: x_i²
                            Q[idx_i, idx_i] += coef
                        else:
                            # Off-diagonal term: x_i*x_j
                            # Split coefficient symmetrically: Q[i,j] = Q[j,i] = coef/2
                            Q[idx_i, idx_j] += 0.5 * coef
                            Q[idx_j, idx_i] += 0.5 * coef
                    
                    # Eigenvalue decomposition to check convexity
                    # Use eigh for symmetric matrices (more robust than eig)
                    eigenvalues, eigenvectors = np.linalg.eigh(Q)
                    
                    # Check for negative eigenvalues (convexity check)
                    # Use numerical tolerance for treating values as zero
                    # Standard numerical tolerance for eigenvalue checks
                    numerical_tolerance = 1e-10
                    
                    # Determine if constraint is convex based on type
                    # For upper bound (expr ≤ ub): convex if Q is PSD (all eigenvalues ≥ 0)
                    # For lower bound (lb ≤ expr): convex if -Q is PSD (all eigenvalues ≤ 0)
                    # For equality: not convex (already excluded)
                    
                    Q_is_psd = not np.any(eigenvalues < -numerical_tolerance)  # Q positive semi-definite
                    Q_is_nsd = not np.any(eigenvalues > numerical_tolerance)   # Q negative semi-definite
                    ...
                    if c.upper is not None and not c.equality:
                        # Upper bound constraint: convex if Q is PSD
                        if Q_is_psd:
                            use_conic_upper = True
                    
                    if c.lower is not None and not c.equality:
                        # Lower bound constraint: convex if -Q is PSD (i.e., Q is NSD)
                        if Q_is_nsd:
                            use_conic_lower = True
                            negate_for_conic = True
                    
                    # Build Cholesky factorization if using conic for either bound
                    if use_conic_upper or use_conic_lower:
                        # For lower bound constraints, we negate Q (so eigenvalues too)
                        if negate_for_conic:
                            Q_for_conic = -Q
                            eigenvalues_for_conic = -eigenvalues
                        else:
                            Q_for_conic = Q
                            eigenvalues_for_conic = eigenvalues
                        
                        # Filter eigenvalues that are significantly positive
                        # (ignore near-zero eigenvalues for numerical stability)
                        positive_mask = eigenvalues_for_conic > numerical_tolerance
                        positive_eigenvalues = eigenvalues_for_conic[positive_mask]
                        positive_eigenvectors = eigenvectors[:, positive_mask]
                        
                        # Build L = V * sqrt(Lambda)
                        # where V is eigenvectors and Lambda is diagonal matrix of eigenvalues
                        # This gives Q_for_conic = L * L^T (Cholesky-like factorization)
                        sqrt_eigenvalues = np.sqrt(positive_eigenvalues)
                        L = positive_eigenvectors @ np.diag(sqrt_eigenvalues)

                    # Determine which expressions to build
                    # - non_conv_expr (v'Qv + c'vy + dy²): for non-convex bounds
                    # - conic_expr_linear (t + c'v + dy): for convex bounds
                    # For range constraints, we may need BOTH if only one bound is convex
                    
                    need_non_convex_expr = False
                    if c.upper is not None and not use_conic_upper:
                        need_non_convex_expr = True
                    if c.lower is not None and not use_conic_lower:
                        need_non_convex_expr = True
                    
                    if need_non_convex_expr:
                        # Build standard expression: v'Qv + c'vy + dy²
                        non_conv_expr = 0
                        ...
                        for i, (var_i, var_j) in enumerate(repn.quadratic_vars):
                            ...
                            if var_i is var_j:
                                ...
                                non_conv_expr += coef * v_i**2
                            else:
                                # For cross-terms x_i*x_j, the exact hull is v_i*v_j
                                non_conv_expr += coef * v_i * v_j
                        
                        # Process linear part (c'x -> c'vy)
                        # Linear terms may be None if expression is purely quadratic
                    ...
                    if use_conic_upper or use_conic_lower:
                        # Convex case: Use conic reformulation
                        # We will build expression with auxiliary variable t
                        # The actual conic constraint will be added directly to the relaxation block
                        
                        # Create auxiliary variable t for conic reformulation
                        ...
                        # Build v vector (disaggregated variables in order)
                        v_vector = [var_substitute_map.get(id(var), var) for var in all_vars]
                        
                        # Build linear expression: t + c'v + dy (or negated if needed)
                        # For upper bound: t + c'v + dy
                        # For lower bound: t - c'v - dy (because we negated the constraint)
                        linear_expr = t
                        
                        # Add linear terms (may be None if expression is purely quadratic)
                        lin_coefs = repn.linear_coefs or []
                        lin_vars = repn.linear_vars or []
                        for coef, var in zip(lin_coefs, lin_vars):
                            v = var_substitute_map.get(id(var), var)
                        ...
                        # Store as conic_expr (will be used for appropriate bounds)
                        conic_expr_linear = linear_expr
                        
                        # Add the conic constraint: ||L^T v||² ≤ t*y
                        # Reformulated as SOC with auxiliary variables (sum of squares <= var²):
                        # z[k] = 2 * (L^T v)_k  (linear constraints)
                        # w = t - y  (linear constraint)
                        # s = t + y  (linear constraint)  
                        # sum(z[k]²) + w² ≤ s²
                        #
                        # This form has only single variables squared, which some solvers
                        # may detect as SOC more reliably.
                        
                        r = L.shape[1]  # number of positive eigenvalues
                        
                        # Create auxiliary variables z[k] for the linear transformation 2*L^T @ v
                        ...
                        # Create auxiliary variable w = t - y
                        w = Var(domain=Reals)
                        w_name = unique_component_name(
                            relaxationBlock,
                            '_conic_aux_w_%s' % c.getname(fully_qualified=True, relative_to=disjunct)
                        )
                        relaxationBlock.add_component(w_name, w)
                        
                        # Create auxiliary variable s = t + y (for RHS to be single var squared)
                        s = Var(domain=NonNegativeReals)
                        s_name = unique_component_name(
                            relaxationBlock,
                            '_conic_aux_s_%s' % c.getname(fully_qualified=True, relative_to=disjunct)
                        )
                        relaxationBlock.add_component(s_name, s)
                        
                        # Add linear constraints: z[k] = 2 * sum_j L[j,k] * v[j]
                        ...
                        for k in range(r):
                            z_k_expr = 2 * sum(L[j, k] * v_vector[j] for j in range(n_vars))
                            z_def_constraints.add(k, z[k] == z_k_expr)
                        
                        # Add linear constraint: w = t - y
                        w_def_name = unique_component_name(
                            relaxationBlock,
                            '_conic_w_def_%s' % c.getname(fully_qualified=True, relative_to=disjunct)
                        )
                        w_def_constraint = Constraint(expr=w == t - y)
                        relaxationBlock.add_component(w_def_name, w_def_constraint)
                        
                        # Add linear constraint: s = t + y
                        s_def_name = unique_component_name(
                            relaxationBlock,
                            '_conic_s_def_%s' % c.getname(fully_qualified=True, relative_to=disjunct)
                        )
                        s_def_constraint = Constraint(expr=s == t + y)
                        relaxationBlock.add_component(s_def_name, s_def_constraint)
                        
                        # Build SOC constraint: sum(z[k]²) + w² ≤ s²
                        # This is the cleanest form: sum of variable squares <= single variable squared
                        soc_lhs = sum(z[k]**2 for k in range(r)) + w**2
                        soc_rhs = s**2
                        ...
                        # Map all the auxiliary constraints
                        for k in range(r):
                            constraint_map.transformed_constraints[c].append(z_def_constraints[k])
                            constraint_map.src_constraint[z_def_constraints[k]] = c
                        constraint_map.transformed_constraints[c].append(w_def_constraint)
                        constraint_map.src_constraint[w_def_constraint] = c
                        constraint_map.transformed_constraints[c].append(s_def_constraint)
                        constraint_map.src_constraint[s_def_constraint] = c
            ...
            if c.lower is not None:
                ...
                # Select appropriate expression for lower bound
                if polynomial_degree == 2 and use_conic_lower:
                    # For conic lower bound, we negated the constraint
                    # Original: c.lower ≤ expr  →  -expr ≤ -c.lower
                    # With conic: linear_expr ≤ -c.lower * y
                    newConsExpr = conic_expr_linear <= -c.lower * y
                elif general_NL:
                    newConsExpr = expr >= c.lower * y
                elif polynomial_degree == 2:
                    # Use standard quadratic expression for lower bound
                    newConsExpr = non_conv_expr >= c.lower * y**2
                else:
                    newConsExpr = expr - (1 - y) * h_0 >= c.lower * y
            ...
            if c.upper is not None:
                ...
                # Select appropriate expression for upper bound
                if polynomial_degree == 2 and use_conic_upper:
                    # For conic upper bound: t + c'v + dy ≤ c.upper * y
                    newConsExpr = conic_expr_linear <= c.upper * y
                elif general_NL:
                    newConsExpr = expr <= c.upper * y
                elif polynomial_degree == 2:
                    # Use standard quadratic expression for upper bound
